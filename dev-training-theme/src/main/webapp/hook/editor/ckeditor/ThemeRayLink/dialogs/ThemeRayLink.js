...
CKEDITOR.dialog.add('ThemeRayLink', function (a) {
	
    var b = CKEDITOR.plugins.ThemeRayLink,
        c = function () {
    ...
    return {
        ...
        contents: [{
            ...
            elements: [{
                ...
                children: [{
                    ...
                    setup: function (E) {
                        if (!this.getDialog()
                            .getContentElement('info', 'linkType')) this.getElement()
                            .show();
                    }
                }, {
                    type: 'button',
                ...
                children: [{
                    ...
                    setup: function (E) {
                        if (E.anchors.length > 0) this.getElement()
                            .show();
                        else this.getElement()
                            .hide();
                    },
                    children: [{
                        type: 'hbox',
                    ...
                    focus: true,
                    setup: function (E) {
                        if (E.anchors.length < 1) this.getElement()
                            .show();
                        else this.getElement()
                            .hide();
                    }
        ...
        {
            id: 'beveledStyles',
            label: 'beveled Styles',
            title:  'beveled Styles',
            elements: [{
                type: 'vbox',
                padding: 1,
                children: [{
                    ...
                    children: [
                        ...
                        items: [
                            ...
                            ['pink button', 'pink'],
                            ['sliver button', 'sliver'],
                            ['icone note button', 'icone-button-note'],
                            ['icone alert button', 'icone-button-alert'],
                            ['icone info button', 'icone-button-info'],
                            ['icone downlaod button', 'icone-button-download'],
                            ['icone tick button', 'icone-button-tick']
        ...
        onOk: function () {
            ...
            switch (G.type || 'url') {
            case 'url':
                var J = G.url && G.url.protocol != undefined ? G.url.protocol : 'http://',
                    K = G.url && G.url.url || '';
                E['data-cke-saved-href'] = K.indexOf('/') === 0 ? K : J + K;
                break;
            case 'anchor':
                var L = G.anchor && G.anchor.name,
                    M = G.anchor && G.anchor.id;
            ...
            case 'email':
                ...
                case '':
                case 'encode':
                    var Q = encodeURIComponent(O.subject || ''),
                        R = encodeURIComponent(O.body || ''),
                        S = [];
                    Q && S.push('subject=' + Q);
                    R && S.push('body=' + R);
                    S = S.length ? '?' + S.join('&') : '';
            ...
            if (G.target) if (G.target.type == 'popup') {
                var U = ["window.open(this.href, '", G.target.name || '', "', '"],
                    V = ['resizable', 'status', 'location', 'toolbar', 'menubar', 'fullscreen', 'scrollbars', 'dependent'],
                    W = V.length,
                    X = function (ai) {
            ...
            if (G.adv) {
                ...
                E['class']=G.adv['advCSSClasses'].concat(' ').concat(G.adv['advThemeCssClasses']).concat(' ').concat(G.adv['advThemeLinkSize']);
            ...
            if (!this._.selectedElement) {
                ...
                var ae = this._.selectedElement,
                    ...
                    ag = ae.getHtml();
                if (CKEDITOR.env.ie && !(CKEDITOR.document.$.documentMode >= 8) && E.name != ae.getAttribute('name')) {
                ...
                if (this.fakeObj) I.createFakeElement(ae, 'cke_anchor', 'anchor')
                    .replace(this.fakeObj);
